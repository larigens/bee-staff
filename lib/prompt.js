    ...
    inquirer
        ...
        .then((answers) => {
            ...
            if (answers.menu === "View all departments") {
                ...
                const sql = 'SELECT roles.id AS ID, roles.title AS JobTitle, roles.salary AS Salary, department.department_name AS Department FROM roles JOIN department ON roles.department_id = department.id ORDER BY roles.id;';
                view(sql);
            }
            // Fix Manager to display the managers name - maybe separate by department?
            else if (answers.menu === "View all employees") {
                const sql = 'SELECT employee.id AS ID, employee.first_name AS FirstName, employee.last_name AS LastName, employee.manager_id AS Manager, roles.title AS JobTitle, department.department_name AS Department FROM employee JOIN roles ON employee.role_id = roles.id JOIN department ON roles.department_id = department.id ORDER BY employee.id;';
                view(sql);
            }
            //========================== Add ==========================//
...
const promptEmployee = (rolesList, managerList) => {
    inquirer
        ...
        .then((answer) => {
            const { employeeId, employeeFirstName, employeeLastName, employeeRole, isManager, hasManager, manager } = answer;
            const sql = `INSERT into employee (id, first_name, last_name, role_id, manager_id) VALUES (?, ?, ?, ?, ?);`;
            let params = [employeeId, employeeFirstName, employeeLastName, roleId, manager];
                    ...
                    // If employee has a manager, find manager's id.
                        ...
                        database.query(`SELECT manager_id FROM employee WHERE CONCAT_WS(' ',first_name, last_name) LIKE ?;`, [manager], (err, data) => {
                            if (err) {
                                ...
                                manager = data.map(id => parseInt(id.manager_id)); // Collects the value of the id and converts it to an integer.
                                add(sql, params, table);
                            }
                        });
                    }
