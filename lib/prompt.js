    ...
    inquirer
        ...
        .then((answers) => {
            ...
            if (answers.menu === "View all departments") {
                ...
                const sql = `SELECT a.id AS ID, a.first_name AS FirstName, a.last_name AS LastName, roles.title AS JobTitle, department.department_name AS Department, roles.salary AS Salary, CONCAT_WS(' ', b.first_name, b.last_name) AS Manager
                 FROM employee a
            ...
            //========================== Update ==========================//
                ...
                database.query('SELECT * FROM employee;', (err, employeeResults) => {
                    if (err) {
                        ...
                        employeeFullNameList = employeeResults.map(emp => emp.first_name + " " + emp.last_name);
                        database.query('SELECT * FROM roles;', (err, rolesResults) => {
                            if (err) {
                                console.error(err)
                            }
