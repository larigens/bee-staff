...
const { viewEmployees, viewEmployeesByManager, viewEmployeesByDepartment, promptEmployee, getEmployeeList, getManagerList, getManagerID, addEmployee, employeeTable, employeeRolePrompt, updateEmployeeRole, updateRoleRequest, employeeManagerPrompt, updateEmployeeManager, updateManagerRequest, updateManagerID, deleteEmployeePrompt, deleteEmployee } = require('./employee.js');
const { viewRoles, viewRolesByDepartment, getRolesList, getRolesID, rolesPrompt, addRole, rolesTable, deleteRolePrompt, deleteRole } = require('./roles.js');
const { viewDepartments, viewBudget, departmentBudgetPrompt, getDepartmentList, getDepartmentID, departmentPrompt, addDepartment, departmentTable, deleteDepartmentPrompt, deleteDepartment } = require('./department.js');
...
const mainMenu = [
    {
        ...
        name: "menu",
        choices: ["View all departments", "View the total utilized budget of a department", "View all roles", "View roles by department", "View all employees", "View employees by manager", "View employees by department", "Add a department", "Add a role", "Add an employee", "Update an employee role", "Update an employee manager", "Delete a department", "Delete a role", "Delete an employee", "Exit"]
    }
]
...
const init = () => {
    inquirer
        ...
        .then((answers) => {
            ...
            else if (answers.menu === "View the total utilized budget of a department") {
                getBudget();
            }
...
async function updateAManager() {
    try {
        ...
        let results = await employeeManagerPrompt(employeeFullNameList, managerList); // Then display them as options for the user and gets the array with the employee's new manager and full name.
...
const updateID = (sql) => {
    database.query(sql, (err, data) => {
        err ? console.error(err) : console.log(" as Manager\n\n----------🐝🍯----------\n");
    })
}
